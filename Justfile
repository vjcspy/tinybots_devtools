...
# Megazord Events
dev-megazord-events:
    {{compose}} run --rm --service-ports --no-deps --use-aliases --entrypoint "sh -c 'corepack enable && yarn start'" megazord-events
...
test-megazord-events:
    ...
    {{compose}} run --rm --no-deps --use-aliases megazord-events
...
test-wonkers-graphql:
    ...
    {{compose}} up -d \
          mysql-typ-e-db typ-e mysql-wonkers-db wonkers-db wonkers wonkers-account
    {{compose}} run --rm --no-deps --use-aliases wonkers-graphql
